...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        if token_scales is not None:
            img_len = token_scales.shape[-1]
            attn_weights[:, :, 36:, 36:36+img_len] = attn_weights[:, :, 36:, 36:36+img_len] + token_scales.log().reshape(bsz, 1, 1, img_len)
        # ================================================================================
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.IMG_LEN = 576
        self.selection = 'mllm'
        self.propagation = 'GraphProp'
        self.token_scale = True
    ...
    def get_graph(self, x: torch.Tensor):
        B, N, C = x.shape

        x_normed = x / x.norm(dim=-1, keepdim=True)
        x_cossim = x_normed @ x_normed.transpose(-1, -2)
        # threshold = torch.kthvalue(x_cossim, N-1-self.num_neighbours, dim=-1, keepdim=True)[0]
        ...
        # t_cossim = attn.transpose(-1, -2) @ attn
        # t_cossim = t_cossim[0]
        # diag = torch.diag(t_cossim)
        # diag = torch.diag_embed(diag)
        # t_cossim = t_cossim - diag
        # t_cossim = t_cossim.unsqueeze(0).expand(B,-1,-1)
        # # print('t_cossim:')
        # # print(t_cossim.shape)
        # # print(t_cossim)

        # graph = 0.7*graph + 0.3*t_cossim
    ...
    def select(self, weight: torch.Tensor, standard: str = "None", num_prop: int = 0):
        ...
        if standard == "IMGAttnMean":
            ...
            # last text token
            token_rank = torch.mean(weight, dim=1)[:, -1, :]

            # mean text token
            # token_rank = torch.mean(weight, dim=1)
            # token_rank = torch.mean(token_rank, dim=1)
        ...
        index_kept = index_kept.sort().values
        index_prop = index_prop.sort().values

        return index_kept, index_prop

    def propagate(self, x: torch.Tensor, weight: torch.Tensor, 
        ...
        if standard == "None":
            ...
            if token_scales is not None:
                token_scales_kept = token_scales.gather(dim=1, index=index_kept) # B, N-1-num_prop
                token_scales_prop = token_scales.gather(dim=1, index=index_prop) # B, num_prop
                weight_prop = weight_prop.to(token_scales_prop.dtype)
    ...
    def forward(
        ...
        if self.token_scale and seq_length > 1:
            token_scales = torch.ones([N])
            token_scales = token_scales.unsqueeze(0).expand(batch_size,-1).to(hidden_states.device)
        else:
            token_scales = None
